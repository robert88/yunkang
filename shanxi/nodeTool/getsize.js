$(function () {
	var x,y,flag
	$("body").append("<canvas id='size' style='position:fixed;left:0;top:0px;display:none;' ></canvas>")
	...
	$("body").append("<div id='text' style='position:fixed;left:0;top:30px;z-index:1;color:red;padding:20px;'><i class='tip' style='display: inline-block;width: 10px;height: 10px;margin-right: 10px;'></i><span class='text'></span></div>")
	$("body").append("<canvas id='color2' style='position:fixed;left:0;top:0px;border:1px solid red;display: none' width=100 height=100></canvas>")
	var c = $("#size")[0];
	...
	function selectcolore(img,x0,y0) {
        ...
        $("#color").find(".text").html("#{0}{1}{2}   rgb({3},{4},{5}) w:{6} h:{7}".tpl(
			pixel[0].toString(16).fill("00"),
			pixel[1].toString(16).fill("00"),
			pixel[2].toString(16).fill("00"),
			pixel[0],
			pixel[1],
			pixel[2]
		))
